...
void COnggum::Time_Update(float deltaTime)
{
...
void COnggum::Render(HDC hDC, Coordi Scroll)
{
	if (m_bDead)
		return;
	m_Info.Update_Domain();
	...
	if (m_FDead.bAct) {
		ytemp = 16;
		if (m_LR < 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 39, 50, +16, ytemp, -PI * m_FDead.iCount / 20);
		else if (m_LR > 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 39, 50, -16, ytemp, -PI * m_FDead.iCount / 20);
	}
	if (m_FMove.bAct) {
		ytemp = 25;
		if (m_LR < 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 1 + m_FMove.iCount * 17, 41, +16, ytemp);
		else if (m_LR > 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 0 + m_FMove.iCount * 17, 41, -16, ytemp);
	}
	else if (m_FSleep.bAct) {
		ytemp = 16;
		if (m_FSleep.iCount >= 95) {
			if (m_LR < 0)
				hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 56, 50, +16, ytemp);
			else if (m_LR > 0)
				hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 55, 50, -16, ytemp);
		}
		else {
			if (m_LR < 0)
				hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 39, 50, +16, ytemp);
			else if (m_LR > 0)
				hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 38, 50, -16, ytemp);
		}
	}
	else if (m_FSlide.bAct) {
		ytemp = 16;
		if (m_LR < 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 77 + m_FSlide.iCount * 17, 50, +16, ytemp);
		else if (m_LR > 0)
			hMonster = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Enemy, 76 + m_FSlide.iCount * 17, 50, -16, ytemp);
...
void COnggum::Release()
{
}

USHORT COnggum::Collision(CObject* other)
{
	USHORT Collision = 0;
	if(m_FDead.bAct)
		return Collision;
...
void COnggum::Do_Dead(float deltaTime)
{
