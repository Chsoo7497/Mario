...
CFireFlower::CFireFlower()
	:m_FNomal({ 0.f, 0.2f, 0, 3, false, 1 })
	, m_FCollision({ 0.f, 0.1f, 0, 20, false, 1 })
	, m_hit(false)
{
	m_ITag = CItem::ID::FIREFLOWER;
...
void CFireFlower::Time_Update(float deltaTime)
{
	if (m_bDead)
		return;
...
void CFireFlower::Render(HDC hDC, Coordi Scroll)
{
	if (m_bDead)
		return;
	m_Info.Update_Domain();
	...
	if (m_FNomal.bAct)
		hItem = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Playable, 1 + 17 * 4 + 17 * m_FNomal.iCount, 597, 16, 16);
...
USHORT CFireFlower::Collision(CObject* other)
{
	USHORT Collision = 0;	
