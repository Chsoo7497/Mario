...
CMushroom::CMushroom()
	...
	, m_FCollision({ 0.f, 0.1f, 0, 20, false, false })
	, m_hit(false)
{
	m_ITag = CItem::ID::MUSHROOM;
	m_Info.Velo = Coordi(-100.f, 0.f);
...
void CMushroom::Time_Update(float deltaTime)
{
	if (m_bDead)
		return;
...
void CMushroom::Render(HDC hDC, Coordi Scroll)
{
	if (m_bDead)
		return;
	m_Info.Update_Domain();
	...
	if (m_FNomal.bAct)
		hItem = CImageManager::Get_Instance()->Find_Sheet(CImageMap::sheet::Playable, 1, 597, 16, 16);
...
USHORT CMushroom::Collision(CObject* other)
{
	USHORT Collision = 0;	
